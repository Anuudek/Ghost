...
// negated forms for the `$not` variants).
function regexToOperator(pattern: RegExp, negated: boolean): {operator: string; value: string} {
    ...
    const base = startsWith && endsWith ? 'contains' : startsWith ? 'starts-with' : endsWith ? 'ends-with' : 'contains';
    const negatedBase = base === 'contains' ? 'does-not-contain' : base === 'starts-with' ? 'does-not-start-with' : 'does-not-end-with';
    const value = source.replace(/^\^/, '').replace(/\$$/, '').replace(/\\([\\.^$|?*+()[\]{}/-])/g, '$1');
    return {operator: negated ? negatedBase : base, value};
}

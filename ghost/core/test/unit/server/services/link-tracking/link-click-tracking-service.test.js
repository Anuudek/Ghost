...
describe('LinkClickTrackingService', function () {
    ...
    describe('bulkEdit', function () {
        ...
        // that survives `new URL()` normalisation (`"` becomes %22, `\` becomes
        // `/`). The url is deserialised from NQL and then re-serialised back
        // into NQL for getFilteredIds, so both hops have to escape it -
        // otherwise the filter fails to parse and the edit silently no-ops.
        [
